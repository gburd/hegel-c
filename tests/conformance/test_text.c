 ...
 * Metrics: {"length": <codepoint_count>}
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * Counts lead bytes (bytes that don't start with 10xxxxxx).
 */
static size_t utf8_codepoint_count(const char *s)
{
    size_t count = 0;
    while (*s) {
        /* A byte is a lead byte if it doesn't match 10xxxxxx */
...
{
    (void)user_data;
    char *text = hegel_draw_string(tc, hegel_text((size_t)g_min_size, (size_t)g_max_size));
    if (!text) {
        fprintf(metrics_file, "{\"length\": 0}\n");
        return;
    }
    size_t cp_count = utf8_codepoint_count(text);
